...
function resolveBotId(botKey) {
  const map = {
    'ai-guide': process.env.COZE_BOT_ID_AI_GUIDE,
    'study-prompt': process.env.COZE_BOT_ID_STUDY_PROMPT,
    'general-prompt': process.env.COZE_BOT_ID_GENERAL_PROMPT,
  };
...
export async function handleCozeRequest(body) {
  ...
  if (!botId) {
    throw new Error(
      'Missing COZE_BOT_ID (or bot-specific COZE_BOT_ID_* env). Add them in Vercel Project Settings → Environment Variables.'
    );
  }
